...
public:
    ...
    void Decode(std::map<Satellite, BaseEphemeris*>& ephemsMap);
    void Close();

private:
    OEM7Msg<Range> preRangeMsg;
    ...
    BestPos BestPosDecode(unsigned char* msg) const;
};
...
template<typename T>
inline void ClearStream(T& stream) {};

template<>
inline void ClearStream(std::stringstream& strStream)
{
    strStream.clear();
    strStream.str("");
    strStream.seekg(0);
...
template<StreamType T>
void OEM7Decoder<T>::Decode(std::map<Satellite, BaseEphemeris *> &ephemsMap)
{
    isAvailable = false;
    while (stream.eof() == false)
    {
        unsigned char sync = 0;
        stream.read(reinterpret_cast<char*>(&sync), 1);
        if (sync == 0xAA)
        {
            stream.read(reinterpret_cast<char*>(&sync), 1);
            if (sync == 0x44)
...
template<>
inline void OEM7Decoder<std::stringstream>::Close()
{
}
...
inline Range OEM7Decoder<T>::RangeDecode(unsigned char* msg) const
{
    ...
    for (unsigned long i = 0; offset <= (range.obsNum - 1) * 44; i++)
    {
        ...
        if (auto [sys, frequency, signalTy, isTrue] = GetSateMap(navisys, signalType); isTrue)
        {
            ...
            obs.parityKnownFlag = BitCut<bool, unsigned long, 1>(trackingStatus, 11);
            range.sateDataMap[sate].obsMap[signalTy] = obs;
        }
        offset += 44;
    }
    return range;
...
template<StreamType T>
BestPos OEM7Decoder<T>::BestPosDecode(unsigned char *msg) const
{
    auto msgData = msg + 28;
    auto latitude = GetField<double>(msgData+ 8);
    auto longitude = GetField<double>(msgData + 16);
    auto height = GetField<double>(msgData + 24) + GetField<float>(msgData + 32);
    return BestPos(latitude, longitude, height);
}
