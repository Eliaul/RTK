...
{
    std::cout << std::format("{} ", spp.isAvailable) << spp.gpsTime << std::format(" X:{:12.4f} Y:{:12.4f} Z:{:12.4f} Vx:{:8.4f} Vy:{:8.4f} Vz:{:8.4f} B:{:12.8f} L:{:12.8f} H:{:8.3f} GPSClk:{:8.3f} BDSClk:{:8.3f} VClk:{:8.3f} PDOP:{:8.3f} Sigma:{:8.3f} VSigma:{:8.3f} GPSSats:{:3} BDSSats:{:3}\n", spp.posXYZ(1), spp.posXYZ(2), spp.posXYZ(3), spp.velXYZ(1), spp.velXYZ(2), spp.velXYZ(3), spp.posBLH(1),
...
//        if (range.gpsTime.GpsSecOfWeek().count() == 379991000000000)
//        {
//            std::cout << B << std::endl << P << std::endl << Qxx << std::endl;
//        }
...
inline void SPPResult<T>::SPVCalculate(Range& range, std::map<Satellite, BaseEphemeris*>& ephems)
{
    ...
    for (auto& [sate, sateData] : range.sateDataMap)
    {
        if (!sateData.isError && !sateData.isLowHeightAngle)
        {
            index++;
