...
void Range::GetSateMotionState(std::map<Satellite, BaseEphemeris*>& ephems)
{
    ...
    {
        if (!sateData.isError && ephems.contains(sate))
        {
...
std::ostream& operator<<(std::ostream& basicOstream, const Range& range)
{
    for (auto& [sate, sateData] : range.sateDataMap)
    {
        if (!sateData.isError && !sateData.isLowHeightAngle)
        {
            ...
            basicOstream << name << std::format("{:02} X={:13.3f} Y={:13.3f} Z={:13.3f} Clk={:15.6E} Vx={:13.4f} Vy={:13.4f} Vz={:13.4f} Clkd={:14.5E} PIF={:15.4f} Trop={:8.3f} E={:8.3f}deg\n", sate.PRN, sateData.satePos_Rz(1), sateData.satePos_Rz(2), sateData.satePos_Rz(3),
